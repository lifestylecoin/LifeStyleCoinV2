...
unsigned int nTargetSpacing     = 5 * 60; // 5 minute block spacing (Old Block Spacing)   
unsigned int nStakeTargetSpacing = 5 * 60; // 5-minute block spacing (New Block Spacing)
unsigned int nStakeMinAge       = 24 * 60 * 60; // 24 hours (Original Minimum Stake Age)
unsigned int nStakeMinAgeV2 = 24 * 60 * 60; // 24 Hour Minimum Stake Age (New Minimum Stake Age)
...
const string strMessageMagic = "LifeStyleCoin Signed Message:\n";
...
// lifestylecoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LifeStyleCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        const char* pszTimestamp = "LifeStyle Time";
        CTransaction txNew;
        txNew.nTime = 1499667081;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x9c1117a666b1a75a1cbb992a5057b65880e082db21dcf6330fbabe36d4bc52be"));
        block.print();
